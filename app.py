...
import os
import razorpay 
RAZORPAY_KEY_ID='rzp_test_74ZJYHzgT7i4S1'
RAZORPAY_KEY_SECRET='C9B3SSdGRWQsnd9KBA1SUdZ3'
...
#For payment---->id,name,price,this is paymentgateway
@app.route('/pay/<itemid>/<name>/<price>',methods=['GET','POST'])
def pay(itemid,name,price):
    try:
        # Get the quantity from the form
        qty=int(request.form['qty'])
        #calculate the total amount in paise (paice is in rupees)
...
def success():
    if session.get('user'):
       ...
       try:
          ...
          cursor=mydb.cursor(buffered=True)
          cursor.execute('insert into orders(itemid,item_name,total_price,user,qty)value(%s,%s,%s,%s,%s)',
          [itemid,name,total_price,session.get('user'),qty])
          mydb.commit()
          cursor.close()
          flash('order placed successfully')
          return 'orders'
       except razorpay.errors.signatureVerificationError:
          return 'payment Verifivation failed!',400 
    else:
        return redirect(url_for('login'))
...
def search():
    if request.method=="POST":
        ...
        data=cursor.fetchall()
        return render_template('dashboard.html',itmes=data)
